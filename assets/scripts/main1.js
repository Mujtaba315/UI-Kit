// Fixed search navbar 7 / 6 / 24 

// Search navbar

// Changing dropdown arrow on hover

$('.navbar-left-content .navbar-dropdown').hover(
    function () {
        $('.navbar-left-content .navbar-dropdown img').attr
            ('src', 'assets/images/Arrow.png').css
            ({ 'width': '8px', 'height': '4px' })
    },

    function () {
        $('.navbar-left-content .navbar-dropdown img').attr
            ('src', 'assets/images/navbar dropdown arrow.png')
    }
);

// Opening dropdown on click

$('.search-navbar .navbar-left-content .navbar-dropdown').click(function () {
    $('.search-navbar .navbar-left-content').toggleClass('open');
});

// Menu dropdown 6 / 6 / 24

$('.search-navbar-menu .search-navbar-menu-items .navbar-menu-dropdown').click(function(){
    $('.search-navbar-menu .search-navbar-menu-items').toggleClass('open');
});

$('.search-navbar .navbar-right-content i').click(function(){
    $('div.search-navbar-menu').toggleClass('open');

    if ( $('div.search-navbar-menu').hasClass('open') ) {
        $('.search-navbar.fixed .navbar-right-content i').css('padding-right', '46.5px');
    } else {
        $('.search-navbar.fixed .navbar-right-content i').css('padding-right', '33px');
    }
});

// $('.search-navbar-menu .search-navbar-menu-close-icon i').click(function(){
//     $('div.search-navbar-menu').removeClass('open');
// });

// Closing navbar menu if screen width > 600

$(document).load($(window).bind("resize", checkPosition));
function checkPosition()
{
    if ($(window).width() > 600){
        $('div.search-navbar-menu').removeClass('open');
    }
}


// 10 / 6 / 24

$('.search-navbar-sidebar .sidebar-items .sidebar-dropdown .sidebar-dropdown-btn').click(function(){
    $(this).toggleClass('open');
});

$('.search-navbar-sidebar .sidebar-items .sidebar-dropdown .sidebar-dropdown-btn').hover(
    function () {
        $('.search-navbar-sidebar .sidebar-items .sidebar-dropdown .sidebar-dropdown-btn img').attr
            ('src', 'assets/images/Arrow.png').css
            ({ 'width': '8px', 'height': '4px' })
    },

    function () {
        $('.search-navbar-sidebar .sidebar-items .sidebar-dropdown .sidebar-dropdown-btn img').attr
            ('src', 'assets/images/navbar dropdown arrow.png')
    }
);

$('.search-navbar .navbar-right-content i').click(function(){
    $('div.search-navbar-sidebar').toggleClass('open');
    if ( $('div.search-navbar-sidebar').hasClass('open') ){
        $('.search-navbar.fixed .navbar-right-content i').css('padding-right', '50px');
        $('body').css('padding-right', '17px');
    } else {
        $('.search-navbar.fixed .navbar-right-content i').css('padding-right', '33px');
        $('body').css('padding-right', '0px');
    }
});


$(document).load($(window).bind("resize", checkPosition));
function checkPosition()
{
    if ($(window).width() > 600){
        $('div.search-navbar-sidebar').removeClass('open');
    }
}